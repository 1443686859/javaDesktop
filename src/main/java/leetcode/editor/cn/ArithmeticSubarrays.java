...
/**
 * @author  YourName
 * @date 2023-03-23 21:53:06
 */
public class ArithmeticSubarrays{
    public static void main(String[] args) {
        Solution solution = new ArithmeticSubarrays().new Solution();
        System.out.println(solution.checkArithmeticSubarrays(new int[]{-3,-6,-8,-4,-2,-8,-6,0,0,0,0}, new int[]{5,4,3,2,4,7,6,1,7}, new int[]{6,5,6,3,7,10,7,4,10}));
   }
...
class Solution {
    ...
    public boolean check(int[] nums, int start, int end) {
        ...
        for (int i = start; i <= end; i++) {
            ...
            if ((nums[i] - min) % d != 0 || count[index] != 0) {
                return false;
            }
            count[index] = 1;
            used++;
        }
        return used == count.length;
    }
}
