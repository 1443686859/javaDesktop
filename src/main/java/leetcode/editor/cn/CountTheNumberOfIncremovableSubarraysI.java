...
输入：nums = [1,2,3,4]
输出：10
解释：10 个移除递增子数组分别为：[1], [2], [3], [4], [1,2], [2,3], [3,4], [1,2,3], [2,3,4] 和 [1
,2,3,4]。移除任意一个子数组后，剩余元素都是递增的。注意，空数组不是移除递增子数组。
...
class Solution {
    public int incremovableSubarrayCount(int[] nums) {
        ...
        for (int j = n - 1; j == n - 1 || nums[j] < nums[j + 1]; j--) {
            while (index >= 0 && nums[index] >= nums[j]) {
                index--;
            }
            result += index + 2;
        }
