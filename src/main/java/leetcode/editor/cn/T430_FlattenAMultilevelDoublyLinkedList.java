...
// 1 <= Node.val <= 10^5 
// 
// Related Topics 深度优先搜索 链表 双向链表 👍 281 👎 0
...
public class T430_FlattenAMultilevelDoublyLinkedList{
    public static void main(String[] args) {
        Solution solution = new T430_FlattenAMultilevelDoublyLinkedList().new Solution();
        // TO TEST
        Node first = new Node();
        first.val = 1;
        ...
        first.child = second;
        second.child = third;
        System.out.println(solution.flatten(first));
    }
    static class Node {
        public int val;
        public Node prev;
...
class Solution {
    ...
    public Node cycle(Node head) {
        ...
        while (currentHead != null) {
            ...
            if (currentHead.child != null) {
                Node newNext = cycle(currentHead.child);
                currentHead.next = currentHead.child;
                currentHead.child.prev = currentHead;
                currentHead.child = null;
                newNext.next = next;
                if (next != null) {
                    next.prev = newNext;
                }
