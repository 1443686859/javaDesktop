...
// 
//
// Related Topics 线段树 队列 数组 动态规划 单调队列 堆（优先队列） 👍 119 👎 0
...
class Solution {
    public int boxDelivering(int[][] boxes, int portsCount, int maxBoxes, int maxWeight) {
        ...
        for (int j = 1; j <= n; j++) {
            int sum = 0;
            for (int i = j; i >= 1 && i >= j - maxBoxes + 1; i--) {
                sum += boxes[i - 1][1];
                if (sum > maxWeight) break;
                dp[j] = Math.min(dp[j], dp[i - 1] + get(boxes, i, j));
            }
        }
        return dp[n];
    }
    public int get(int[][] boxes, int i, int j) {
        int result = 2;
        int start = boxes[i - 1][0];
        for (int cur = i; cur <= j; cur++) {
            if (start == boxes[cur - 1][0]) {
                continue;
            }
            result++;
            start = boxes[cur - 1][0];
