...
// 
//
// Related Topics 深度优先搜索 广度优先搜索 并查集 数组 矩阵 👍 133 👎 0
...
class Solution {
    ...
    public int[] pondSizes1(int[][] land) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (land[i][j] == 0) {
                    if (i > 0 && land[i - 1][j] == 0) {
                        merge(parent, (i - 1)* m + j, i * m + j);
                    }
                    if (j > 0 && land[i][j - 1] == 0) {
                        merge(parent, (i)* m + j - 1, i * m + j);
                    }
                    if (j > 0 && i > 0 && land[i - 1][j - 1] == 0) {
                        merge(parent, (i - 1)* m + j - 1, i * m + j);
                    }
                    if (i > 0 && j < m - 1&& land[i - 1][j + 1] == 0) {
                        merge(parent, (i - 1)* m + j + 1, i * m + j);
                    }
                }
            }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (land[i][j] == 0) {
                    int current = getParent(parent, i * m + j);
    ...
    public int getParent(int[] parent, int j) {
        if (parent[j] != j) {
    ...
    public int dfs(int i, int j, int[][] land, boolean[][] used) {
        if (i < 0 || j < 0 || i >= land.length || j >= land[0].length || land[i][j] != 0 || used[i][j] ) {
            return 0;
        }
        int result = 1;
        used[i][j] = true;
