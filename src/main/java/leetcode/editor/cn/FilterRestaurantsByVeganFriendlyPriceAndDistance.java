...
class Solution {
    public List<Integer> filterRestaurants(int[][] restaurants, int veganFriendly, int maxPrice, int maxDistance) {
        ...
        for (int[] r : restaurants) {
            if ((veganFriendly == 0 || (veganFriendly == 1 && veganFriendly == r[2])) && maxPrice >= r[3] && maxDistance >= r[4]) {
                result.add(index);
            }
            index++;
        }
        Collections.sort(result, (n, o) -> {
            if (restaurants[o][1] - restaurants[n][1] == 0) {
                return restaurants[o][0] - restaurants[n][0];
            } else {
                return restaurants[o][1] - restaurants[n][1];
            }
        });
        return result.stream().map(it -> {return restaurants[it][0];}).collect(Collectors.toList());
    }
}
