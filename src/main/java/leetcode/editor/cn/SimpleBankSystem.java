...
class Bank {
    ...
    public Bank(long[] balance) {
        current = balance;
        maxLength = balance.length;
    }
    
    public boolean transfer(int account1, int account2, long money) {
        // check账户
        if (account1 > maxLength || account2 > maxLength || account1 <= 0 || account2 <= 0) {
            return false;
        }
        if (current[account1 - 1] < money) {
            return false;
        }
        current[account1 - 1] -= money;
        current[account2 - 1] += money;
        return true;
    }
    
    public boolean deposit(int account, long money) {
        if (account <= 0 || account > maxLength) {
            return false;
        }
        current[account - 1] += money;
        return true;
    }
