...
// grid[i][j] 为 1 或 -1 
// 
// Related Topics 深度优先搜索 数组 动态规划 矩阵 模拟 👍 96 👎 0
...
public class T1706_WhereWillTheBallFall{
    public static void main(String[] args) {
        Solution solution = new T1706_WhereWillTheBallFall().new Solution();
        // TO TEST
    }
...
//        int start = 0;
        ...
        for (int i = 0 ; i < n; i++) {
            ...
            int start = 0;
            while (start < m) {
                int temp = currentResult + grid[start][currentResult];
                if (temp == n || temp == -1) {
                    currentResult = -1;
                    break;
                }
                if (grid[start][currentResult] *grid[start][temp] == -1) {
                    currentResult = -1;
                    break;
                }
                start++;
