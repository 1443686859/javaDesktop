...
// 
//
// Related Topics 数组 哈希表 字符串 计数 👍 131 👎 0
...
class Solution {
    ...
    public void add(Map<String, Integer> countMap, int count, String domain) {
        for (int i = domain.length() - 1; i >= 0; i--) {
            if (domain.charAt(i) == '.') {
                countMap.put(domain.substring(i + 1), countMap.getOrDefault(domain.substring(i + 1), 0) + count);
            }
        }
